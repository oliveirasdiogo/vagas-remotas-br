...
export default function App() {
  ...
  const visible = useMemo(() => jobs.filter(job => {
    ...
    return true
  }).sort((a, b) => sort === 'company' ? a.company.localeCompare(b.company) : sort === 'date' ? jobDate(b) - jobDate(a) : b.relevance - a.relevance), [jobs, hidden.value, workMode, query, area, source, onlyUnseen, view, visited.value, favorites.value, sort])
  ...
  return <div className={`appShell ${collapsed ? 'collapsed ' : ''}${mobile ? 'menuOpen' : ''}`}>
    ...
    <main className="content">
      <div className="mobileTop"><button onClick={() => setMobile(value => !value)}>Filtros ☰</button></div>
      ...
      <div className="sectionHead"><div><h2>{sectionTitle}</h2><p>{view === 'overview' ? 'Oportunidades mais relevantes para o seu perfil' : `${visible.length.toLocaleString('pt-BR')} resultados`}</p></div><select value={sort} onChange={event => { setSort(event.target.value); setPage(1) }} aria-label="Ordenar vagas"><option value="relevance">Relevância</option><option value="date">Mais recentes</option><option value="company">Empresa</option></select></div>
