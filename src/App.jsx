...
export default function App() {
  ...
  const visible = useMemo(() => jobs.filter(job => !hidden.value.has(job.id) && (workMode === 'all' || job.workMode === workMode) && (!query || [job.title, job.company, job.area, job.location, ...job.tags].join(' ').toLowerCase().includes(query.toLowerCase())) && (!area || job.area === area) && (!source || job.source === source) && (!onlyUnseen || !visited.value.has(job.id)) && (listMode !== 'visited' || visited.value.has(job.id)) && (listMode !== 'favorites' || favorites.value.has(job.id))).sort((a,b) => sort === 'company' ? a.company.localeCompare(b.company) : sort === 'date' ? jobDate(b) - jobDate(a) : b.relevance - a.relevance), [jobs, hidden.value, workMode, query, area, source, onlyUnseen, listMode, visited.value, favorites.value, sort])
  ...
  return <div className={`${collapsed ? 'collapsed ' : ''}${mobile ? 'menuOpen' : ''}`}>
    ...
    <main><div className="mobileTop"><div className="brand">VagasRemotasBR</div><button onClick={() => setMobile(v => !v)}>Filtros ☰</button></div>
      ...
      <section className="stats"><div className="stat accent"><span>Novas hoje</span><strong>{jobs.filter(j => jobAgeDays(j) === 0 && !hidden.value.has(j.id)).length}</strong></div><button type="button" className={`stat interactive ${listMode === 'visited' ? 'active' : ''}`} aria-pressed={listMode === 'visited'} onClick={() => { setListMode(mode => mode === 'visited' ? 'all' : 'visited'); setOnlyUnseen(false); setPage(1) }}><span>Visualizadas</span><strong>{jobs.filter(j => visited.value.has(j.id) && !hidden.value.has(j.id)).length}</strong></button><button type="button" className={`stat interactive ${listMode === 'favorites' ? 'active' : ''}`} aria-pressed={listMode === 'favorites'} onClick={() => { setListMode(mode => mode === 'favorites' ? 'all' : 'favorites'); setOnlyUnseen(false); setPage(1) }}><span>Favoritas</span><strong>{jobs.filter(j => favorites.value.has(j.id) && !hidden.value.has(j.id)).length}</strong></button><div className="stat"><span>Disponíveis</span><strong>{jobs.filter(j => j.status === 'active').length}</strong></div></section>
