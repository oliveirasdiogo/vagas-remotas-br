...
export default function App() {
  ...
  const [onlyUnseen, setOnlyUnseen] = useState(false), [listMode, setListMode] = useState('all'), [collapsed, setCollapsed] = useState(false), [mobile, setMobile] = useState(false)
  ...
  const visible = useMemo(() => jobs.filter(job => !hidden.value.has(job.id) && (workMode === 'all' || job.workMode === workMode) && (!query || [job.title, job.company, job.area, job.location, ...job.tags].join(' ').toLowerCase().includes(query.toLowerCase())) && (!area || job.area === area) && (!source || job.source === source) && (!onlyUnseen || !visited.value.has(job.id)) && (listMode !== 'new' || jobAgeDays(job) === 0) && (listMode !== 'visited' || visited.value.has(job.id)) && (listMode !== 'favorites' || favorites.value.has(job.id)) && (listMode !== 'available' || job.status === 'active')).sort((a,b) => sort === 'company' ? a.company.localeCompare(b.company) : sort === 'date' ? jobDate(b) - jobDate(a) : b.relevance - a.relevance), [jobs, hidden.value, workMode, query, area, source, onlyUnseen, listMode, visited.value, favorites.value, sort])
  ...
  return <div className={`${collapsed ? 'collapsed ' : ''}${mobile ? 'menuOpen' : ''}`}>
    <aside><div className="brandRow"><div className="brand">VagasRemotasBR</div><button onClick={() => setCollapsed(v => !v)} title="Recolher menu">☰</button></div><p className="about">Seu painel pessoal de vagas remotas disponíveis para profissionais no Brasil.</p>
      ...
      <div className="checks"><label><input checked={onlyUnseen} onChange={e => { setOnlyUnseen(e.target.checked); if (e.target.checked && listMode === 'visited') setListMode('all'); setPage(1) }} type="checkbox" /> Apenas não visualizadas</label><label><input checked={listMode === 'favorites'} onChange={e => { setListMode(e.target.checked ? 'favorites' : 'all'); setOnlyUnseen(false); setPage(1) }} type="checkbox" /> Apenas favoritas</label></div>
      <div className="asideBottom"><div className="update">Última coleta<br/><b>{updated}</b></div></div>
    </aside>
    <main><div className="mobileTop"><div className="brand">VagasRemotasBR</div><button onClick={() => setMobile(v => !v)}>Filtros ☰</button></div>
      ...
      <section className="stats"><button type="button" className={`stat interactive accent ${listMode === 'new' ? 'active' : ''}`} aria-pressed={listMode === 'new'} onClick={() => { setListMode(mode => mode === 'new' ? 'all' : 'new'); setOnlyUnseen(false); setPage(1) }}><span>Novas hoje</span><strong>{jobs.filter(j => jobAgeDays(j) === 0 && !hidden.value.has(j.id)).length}</strong></button><button type="button" className={`stat interactive ${listMode === 'visited' ? 'active' : ''}`} aria-pressed={listMode === 'visited'} onClick={() => { setListMode(mode => mode === 'visited' ? 'all' : 'visited'); setOnlyUnseen(false); setPage(1) }}><span>Visualizadas</span><strong>{jobs.filter(j => visited.value.has(j.id) && !hidden.value.has(j.id)).length}</strong></button><button type="button" className={`stat interactive ${listMode === 'favorites' ? 'active' : ''}`} aria-pressed={listMode === 'favorites'} onClick={() => { setListMode(mode => mode === 'favorites' ? 'all' : 'favorites'); setOnlyUnseen(false); setPage(1) }}><span>Favoritas</span><strong>{jobs.filter(j => favorites.value.has(j.id) && !hidden.value.has(j.id)).length}</strong></button><button type="button" className={`stat interactive ${listMode === 'available' ? 'active' : ''}`} aria-pressed={listMode === 'available'} onClick={() => { setListMode(mode => mode === 'available' ? 'all' : 'available'); setOnlyUnseen(false); setPage(1) }}><span>Disponíveis</span><strong>{jobs.filter(j => j.status === 'active' && !hidden.value.has(j.id)).length}</strong></button><button type="button" className="stat interactive restoreStat" onClick={hidden.clear} disabled={hidden.value.size === 0} title="Restaurar todas as vagas ocultadas"><span>Ocultas</span><strong>{hidden.value.size}</strong></button></section>
      ...
      <footer className="siteFooter"><span>Projeto pessoal de vagas remotas para o Brasil.</span><a href="https://github.com/oliveirasdiogo" target="_blank" rel="noopener noreferrer" aria-label="GitHub de Diogo Oliveira"><svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 .7A11.3 11.3 0 0 0 8.4 22.8c.6.1.8-.3.8-.6v-2.2c-3.4.7-4.1-1.4-4.1-1.4-.5-1.4-1.3-1.7-1.3-1.7-1.1-.7.1-.7.1-.7 1.2.1 1.8 1.2 1.8 1.2 1.1 1.8 2.8 1.3 3.5 1 .1-.8.4-1.3.8-1.6-2.7-.3-5.5-1.3-5.5-6A4.7 4.7 0 0 1 6.4 8c-.1-.3-.5-1.6.1-3.2 0 0 1-.3 3.3 1.2a11.5 11.5 0 0 1 6 0c2.3-1.5 3.3-1.2 3.3-1.2.6 1.6.2 2.9.1 3.2a4.7 4.7 0 0 1 1.2 3.3c0 4.6-2.8 5.7-5.5 6 .4.4.8 1.1.8 2.2v3.2c0 .4.2.7.8.6A11.3 11.3 0 0 0 12 .7Z"/></svg><span>oliveirasdiogo</span></a></footer>
