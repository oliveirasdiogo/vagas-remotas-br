...
export default function App() {
  ...
  useEffect(() => { if (!mobile) return undefined; const close = event => { if (event.key === 'Escape') setMobile(false) }; document.addEventListener('keydown', close); return () => document.removeEventListener('keydown', close) }, [mobile])
  ...
  return <div className={`appShell ${collapsed ? 'collapsed ' : ''}${mobile ? 'menuOpen' : ''}`}>
    <aside className="sidebar">
      ...
      <div className="sideFilters"><label>Buscar</label><input value={query} onChange={event => { setQuery(event.target.value); setPage(1) }} onKeyDown={event => { if (event.key === 'Enter') setMobile(false) }} placeholder="Cargo, empresa, tecnologia"/><label>Área</label><select value={area} onChange={event => { setArea(event.target.value); setPage(1); setMobile(false) }}><option value="">Todas as áreas</option>{areas.map(([name]) => <option key={name}>{name}</option>)}</select><label>Plataforma</label><select value={source} onChange={event => { setSource(event.target.value); setPage(1); setMobile(false) }}><option value="">Todas as plataformas</option>{sources.map(([name]) => <option key={name}>{name}</option>)}</select><label className="check"><input checked={onlyUnseen} onChange={event => { setOnlyUnseen(event.target.checked); setPage(1); setMobile(false) }} type="checkbox"/> Apenas não visualizadas</label></div>
    ...
    <main className="content">
      {!mobile && <div className="mobileTop"><button className="mobileMenuButton" onClick={() => setMobile(true)} aria-label="Abrir menu"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 7h14M5 12h14M5 17h14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg><small>Menu</small></button><strong className="mobileBrand">Vagas BR</strong><button className="mobileThemeButton" onClick={toggleTheme} aria-label={dark ? 'Usar modo claro' : 'Usar modo escuro'}>{dark ? '☀' : '☾'}</button></div>}
